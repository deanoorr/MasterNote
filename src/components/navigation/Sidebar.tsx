...
const Sidebar = ({ isOpen, isMobile, onClose }: SidebarProps) => {
  ...
  const renderMobileBackdrop = () => {
    if (isMobile && isOpen) {
      return (
        <div 
          className="fixed inset-0 z-20 bg-secondary-900/50 transition-opacity"
          onClick={onClose}
        />
      );
    }
  ...
  return (
    <>
      ...
      {!isMobile && (
        <div className="fixed inset-y-0 left-0 z-30 w-64 bg-white dark:bg-secondary-800 shadow-lg flex flex-col">
          <div className="flex items-center justify-between px-4 py-5 border-b border-secondary-200 dark:border-secondary-700">
            <h1 className="text-xl font-bold text-primary-600 dark:text-primary-400">MasterNote</h1>
          </div>
